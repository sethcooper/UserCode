...
#include "Analyzers/EcalCosmicTimingAnalyzer/interface/EcalCosmicTimingAnalyzer.h"
...
EcalCosmicTimingAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  {
    LogWarning("EcalCosmicTimingAnalyzer") << "l1MuGMTReadoutCollection" << " not available";
    //return;
  }
  ...
  if (!(L1GTRR.isValid()))
  {
    LogWarning("EcalCosmicTimingAnalyzer") << l1GTReadoutRecTag_ << " not available";
    //return;
  }
  else if(sizeOfDecisionWord<128)
  {
    LogWarning("EcalCosmicTimingAnalyzer") << "size of L1 decisionword is " << sizeOfDecisionWord
  ...
  for (reco::SuperClusterCollection::const_iterator clus = clusterCollection_p->begin(); clus != clusterCollection_p->end(); ++clus)
  {
    ...
    // print out some info
    //     LogWarning("EcalCosmicTimingAnalyzer") << "hit! " << " amp " << ampli  << " : " 
  ...
  if ( recoTracks.isValid() ) {
    //    LogWarning("EcalCosmicTimingAnalyzer") << "... Valid TrackAssociator recoTracks !!! " << recoTracks.product()->size();
    ...
    //    LogWarning("EcalCosmicTimingAnalyzer") << "!!! No TrackAssociator recoTracks !!!";
