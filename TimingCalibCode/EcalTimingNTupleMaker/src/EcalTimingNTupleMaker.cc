...
//========================================================================
EcalTimingNTupleMaker::EcalTimingNTupleMaker( const edm::ParameterSet& iConfig )
//========================================================================
{
   ...
   recHitCollection_   = iConfig.getParameter<edm::InputTag>("recHitCollection");
   ...
   f_ = new TFile(rootfile_.c_str(),"RECREATE");
   f_->cd();
...
//========================================================================
EcalTimingNTupleMaker::~EcalTimingNTupleMaker()
//========================================================================
{
...
//========================================================================
void
EcalTimingNTupleMaker::beginJob(edm::EventSetup const& eventSetup ) {
//========================================================================
...
//========================================================================
void EcalTimingNTupleMaker::endJob() {
//========================================================================
// produce offsets for each TT 
  ...
  for(int dcc = 1; dcc <55; ++dcc){
    ...
    if( absoluteTimingConv_[dcc-1][0]->GetEntries() <= 0 ){
      edm::LogWarning("EcalTimingNTupleMaker")<<"The profile is empty !! For FED  " << fed;
    }
	...
	if( rmsr2 > 0) {RMSr[u] = sqrt(rmsr2);}
      ...
      edm::LogInfo("EcalTimingNTupleMaker")<<"SM " << (dcc+600) << "  " << u+1<<"  "<< mean[u] << "  "<< RMS[u]<<"  " <<meanr[u] << "  "<< RMSr[u];
    }
...
//========================================================================
void
EcalTimingNTupleMaker::analyze(  edm::Event const& iEvent,  edm::EventSetup const& iSetup ) {
//========================================================================
   if (fromfile_) return;
   using namespace edm;
   using namespace cms;
   ...
   if (!DCCHeaders.isValid()) {
	edm::LogError("EcalTimingNTupleMaker") << "can't get the product for EcalRawDataCollection";
   }
  ...
  for ( EcalRawDataCollection::const_iterator headerItr= DCCHeaders->begin();headerItr != DCCHeaders->end(); 
    ...
    LogDebug("EcalTimingNTupleMaker") << " Lambda " << lambda; //hmm... this isn't good, I should keep a record of the wavelength in the headers as an inactive SM might have a different wavelength for this field and make this not go through.
  }
  ///if(lambda <0 || lambda > 3){LogDebug ("EcalTimingNTupleMaker")<<"Stopping: Wrong value for laser wavelength: "<<lambda<<std::endl; return;}//TAKEN OUT TO RUN ON TEST PULSES
   ...
   if (!phits.isValid()){
	edm::LogError("EcalTimingNTupleMaker") << "can't get the product for " << hitCollection_;
        return;
   }
   Handle<EcalRecHitCollection> EBhits;
   Handle<EcalRecHitCollection> EEhits;
   iEvent.getByLabel(recHitCollection_, EBhits);
   iEvent.getByLabel(recHitCollectionEE_, EEhits);
   if(!EBhits.isValid())
   {
     edm::LogError("EcalTimingNTupleMaker") << "can't get the product for " << recHitCollection_;
     return;
   }
   if(!EEhits.isValid())
   {
     edm::LogError("EcalTimingNTupleMaker") << "can't get the product for " << recHitCollectionEE_;
     return;
   }
   ...
   if(ievt_%100 ==0){LogInfo("EcalTimingNTupleMaker") <<"Event: "<<ievt_<< "# of EcalUncalibratedRecHits hits: " << hits->size();}
   ...
   if (correctAVE_) {
      for(EcalUncalibratedRecHitCollection::const_iterator ithit = hits->begin(); ithit != hits->end(); ++ithit) {
        EcalRecHitCollection::const_iterator thisIt =  EBhits->find(ithit->id());
        if(thisIt==EBhits->end())
          continue;
     
        EBDetId anid(ithit->id()); 
     if(anid.ieta()==45 && anid.iphi()==343)
       continue;
     if(anid.ieta()==68 && anid.iphi()==262)
       continue;
     //else if(anid.ieta()==66 && anid.iphi()==322)
     //  continue;
   ...
   {
     numberinaveEB = hits->size();
     averagetimeEB = 5.0;
   }

   //XXX: SIC -- set some tree vars
   numEBcrys_ = numberinaveEB;
   correctionToSample5EB_ = 5.0 - averagetimeEB;
   ...
   for(EcalUncalibratedRecHitCollection::const_iterator ithit = hits->begin(); ithit != hits->end(); ++ithit) {
     ...
     //debug
     //if(anid.ieta()==68)
     //  std::cout << "IETA=" << anid.ieta() << " IPHI=" << anid.iphi() << std::endl;
     
	 EcalElectronicsId elecId = ecalElectronicsMap_->getElectronicsId(anid);
     int DCCid = elecId.dccId();
     int SMind = anid.ic();
     
     LogInfo("EcalTimingNTupleMaker")<<"SM " << DCCid+600 <<" SMind " << SMind << " Chi sq " << ithit->chi2() << " ampl " << ithit->amplitude() << " lambda " << lambda << " jitter " << ithit->jitter();
	 ...
	 if(ithit->chi2()> -1. && ithit->chi2()<10000. && ithit->amplitude()> ampl_thr_ ) { // make sure fit has converged 
	  ...
	  amplProfileConv_[DCCid-1][lambda]->Fill(SMind,ithit->amplitude());
	   ...
	   chTimingEtaRel_->Fill(anid.ieta(),mytime-sMAves_[DCCid-1]+5.0);  
           ...
           //XXX: SIC -- for the tree
           //debug
           //std::cout << "FILL THE TREE EB!" << std::endl;
           cryHashesEB_[cryCounter] = anid.hashedIndex();
           //debug
           if(anid.hashedIndex() > 100000)
             std::cout << "hit eta:" << anid.ieta() << " hit phi: " 
               << anid.iphi() << " hit hash: " << anid.hashedIndex() << std::endl;
           cryTimesEB_[cryCounter] = 25*(mytime-5);
           cryTimeErrorsEB_[cryCounter] = 25*(ithit->chi2());
           cryAmpsEB_[cryCounter] = ithit->amplitude();
   ...
   if (!phitsEE.isValid()){
	edm::LogError("EcalTimingNTupleMaker") << "can't get the product for " << hitCollectionEE_;
   }
   
   // loop over hits
   const EcalUncalibratedRecHitCollection* hitsEE = phitsEE.product(); // get a ptr to the product
   
   if(ievt_%100 ==0){LogInfo("EcalTimingNTupleMaker") <<"Event: "<<ievt_<< " # of EcalUncalibratedRecHits hits: " << hitsEE->size();}
   ...
   //XXX: SIC -- set some tree vars
   numEEcrys_ = numberinaveEEP+numberinaveEEM;
   correctionToSample5EEM_ = 5.0 - averagetimeEEM;
   correctionToSample5EEP_ = 5.0 - averagetimeEEP;
   ...
   for(EcalUncalibratedRecHitCollection::const_iterator ithit = hitsEE->begin(); ithit != hitsEE->end(); ++ithit) {
	 ...
	 if(ithit->chi2()> -1. && ithit->chi2()<10000. && ithit->amplitude()> ampl_thr_ ) { // make sure fit has converged 
	  ...
	  if (correctAVE_) 
          {
            if(anid.zside()==1)
              mytime += 5.0 - averagetimeEEP;
            else
              mytime += 5.0 - averagetimeEEM;
          }
      lasersPerEvt->Fill(ievt_);
	  amplProfileConv_[DCCid-1][lambda]->Fill(SMind,ithit->amplitude());
       absoluteTimingConv_[DCCid-1][lambda]->Fill(SMind,mytime);
       ...
       if(lambda == 0){
	       ...
	       fullAmpProfileEEM_->Fill(anid.ix(),anid.iy(),ithit->amplitude());
                   ...
                   //XXX: SIC -- for the tree
                   //debug
                   //std::cout << "FILL THE TREE EE!" << std::endl;
                   cryHashesEE_[cryCounter] = anid.hashedIndex();
                   cryTimesEE_[cryCounter] = 25*(mytime-5);
                   cryTimeErrorsEE_[cryCounter] = 25*(ithit->chi2());
                   cryAmpsEE_[cryCounter] = ithit->amplitude();
   ...
   //SIC
   //debug
   std::cout << "Fatal root error before Fill()?" << std::endl;
   std::cout << "cryhashesEB[0]" << "\t" << "cryHashesEE_[0]" << std::endl;
   std::cout << cryHashesEB_[0] << "\t" << cryHashesEE_[0] << std::endl;
   std::cout << "cryhashesEB[1]" << "\t" << "cryHashesEE_[1]" << std::endl;
   std::cout << cryHashesEB_[1] << "\t" << cryHashesEE_[1] << std::endl;
   eventTimingInfoTree_->Fill();
...
double EcalTimingNTupleMaker::timecorr(const CaloSubdetectorGeometry *geometry_p, DetId id)
{
   double time = 0.0;
