...
fitPNoMSlice4()
{
  ...
  myFunc2->SetParameters(3,-30,84,1e5,3,0.25,25);
  // Turn off gaussian part
  myFunc2->FixParameter(3,0);
  myFunc2->SetLineColor(4);
  histForPDFNoMPBinsIndex4->Fit("myFunc2","RL+");
  ...
  {
    double funcValBinCenter = myFunc2->Eval(histForPDFNoMPBinsIndex4->GetBinCenter(binx));
    double binEntries = histForPDFNoMPBinsIndex4->GetBinContent(binx);
